...
int main() {
    ...
    std::cout << "Before flip: V=" << mesh.get_vertices().size()
              << " E=" << mesh.get_edges().size()
              << " F=" << mesh.get_faces().size() << '\n';

    const auto edge_ab = find_edge_between(mesh, a, b);
    if (!edge_ab || !mesh.can_flip(edge_ab)) {
        std::cerr << "Edge AB is not flippable\n";
        return 1;
    }
    ...
    std::cout << "After flip: V=" << mesh.get_vertices().size()
              << " E=" << mesh.get_edges().size()
              << " F=" << mesh.get_faces().size() << '\n';
    std::cout << "Has edge CD: " << (find_edge_between(mesh, c, d) ? "yes" : "no") << '\n';
